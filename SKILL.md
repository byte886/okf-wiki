---
name: okf-wiki
description: 用 OKF（Open Knowledge Format v0.2）在本地项目建维护结构化 Markdown 知识库/跨会话外置记忆。当用户要把资料沉淀为知识包 bundle、给项目建长期记忆或上下文文档、新会话恢复项目上下文、对知识库做 ingest 摄入/query 查询/lint 自检，或提到 OKF / Open Knowledge Format / Karpathy LLM Wiki / concept/frontmatter 知识格式时使用。内核是 Google Cloud OKF v0.2（逐字内置不改写），本技能只提供中文使用层、模板与校验。注意：本技能是本地目录技能（~/Doubao/skills/okf-wiki），不是飞书知识库/云文档，命中时直接读本目录，勿去飞书搜同名。
compatibility: 方法论与随附校验脚本均跨平台：okf_validate.py 只用 Python 标准库、不调用任何操作系统专有命令，Windows/Linux/macOS 均可直接运行；已在 macOS 实测，其余平台遇路径/编码差异再就地适配，无需按操作系统拦截停跑
---
...
- **与 Karpathy LLM Wiki 的关系**：LLM Wiki 是理念（知识"编译一次、持续维护、复利增长"）；OKF 是把这个理念落成可移植、跨工具的**格式标准**。本技能用 OKF 落地该理念。
- **形态边界**：豆包 / Trae 没有 Claude Code 那样的后台钩子，**只支持"软模式"**（项目 AGENTS 写约定 + 用户话术触发），做不到保存即自动校验/阻断收尾；不是常驻后台服务。理由见 `reference/memory-handoff.zh.md` §6。
- **在哪里（避免找错，第一跳就读对）**：本技能就是**本地目录** `~/Doubao/skills/okf-wiki/`，**不是飞书知识空间、不是云文档**。一旦提到 okf-wiki / OKF，直接用本地文件工具读本目录，**不要先去飞书知识库或云盘搜索同名对象**。

## 1. 第一原则（强制）

1. **内核不改**：不修改 `OKF-SPEC-v0.2.md`；解释、中文说明、裁剪建议一律写到本技能其它文件。跟版升级 = 整体替换该 SPEC 文件。
2. **包装不裁剪、三档选用**：不删除 OKF 任何字段；"项目按需使用"通过最小/标准/完整三档实现（见 `reference/bundle-files-and-pitfalls.zh.md`），而不是改 SPEC。
...
- **index.md**：目录页，支持"渐进式披露"；**log.md**：倒序变更时间线。二者是保留文件名。
- **链接用普通 Markdown 链接**（推荐 bundle 根相对 `/x/y.md`），**不用 Obsidian `[[双链]]`**；允许死链（代表"尚未写"）。
...
| 完整档 | 标准档 + `type: Attested Computation` | 仅财务/核心指标等"数字必须运行经批准代码"的强一致场景 |

字段逐项解释、index/log 写法、示例与误区见 [reference/bundle-files-and-pitfalls.zh.md](reference/bundle-files-and-pitfalls.zh.md) 与 [reference/field-guide.zh.md](reference/field-guide.zh.md)。
...
4. **篇内既有正文结构一律不动**。若 frontmatter 与正文"信息块"内容重复，指定 **frontmatter 为机器权威、正文信息块由其派生**，并用校验器核对二者一致，避免两处维护漂移。
...
| 需求 | 读这个 |
|---|---|
| 字段含义、最小骨架、YAML 示例 | `reference/field-guide.zh.md` |
| 三档选用、index.md/log.md 写法、链接规则、常见误区 | `reference/bundle-files-and-pitfalls.zh.md` |
...
- 校验器参考最成熟的现成实现 `scaccogatto/okf-skills`（MIT）的一致性思路**零依赖重写**；吸收其"实质改动 verified 不延续、批量回填不冒充人工、覆盖率对账、软/强制维护模式"等**纪律**，不搬它的插件/Action/MCP/可视化/git 事件溯源。
